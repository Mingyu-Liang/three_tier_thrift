...
class service_3Handler : public service_3If {
private:
  std::default_random_engine _gen;
  std::lognormal_distribution<double> _dist;
  double _scaler;
  rpc_params *_rpc_3_params;
  std::lognormal_distribution<double> _rpc_3_proc_dist;

public:
  explicit service_3Handler(rpc_params *rpc_3_params, double scaler) {
    _scaler = scaler;
    auto seed =
    ...
    double rpc_3_proc_time_mean = _rpc_3_params->proc_time_mean;
    if (rpc_3_proc_time_mean != 0) {
      double rpc_3_proc_time_std = _rpc_3_params->proc_time_std;
  ...
  ~service_3Handler() override = default;

  void rpc_3(const std::map<std::string, std::string> &carrier) override {
    ...
    double proc_time;
    double proc_time_mean = _rpc_3_params->proc_time_mean;
    if (proc_time_mean != 0) {
      DistributionType distribution_type = _rpc_3_params->distribution_type;
      switch (distribution_type) {
      case constant:
        proc_time = proc_time_mean;
        break;
      case log_normal:
        proc_time = _rpc_3_proc_dist(_gen);
        break;
      default:
        proc_time = proc_time_mean;
      }
...
void startServer(TServer &server) {
  cout << "Starting the service_3 server..." << endl;
  server.serve();
  cout << "Done." << endl;
}
...
int main(int argc, char *argv[]) {
  ...
  double scaler = services_json["service_3"]["scaler"];
  int port = services_json["service_3"]["server_port"];
  int poolSize = 50;
  TThreadedServer server(
      stdcxx::make_shared<service_3Processor>(
          stdcxx::make_shared<service_3Handler>(&rpc_3_params, scaler)),
